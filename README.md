...
- Includes monitor-first UI modules:
  ...
  - dashboard session quick actions (`rename`, `share`, `unshare`, `summarize`, `archive`, `delete`) with share-link copy support,
  ...
  - richer permission/question queue context (patterns, metadata, tool call ids, per-question details),
  - question reply templates (`empty` / `first options`) for safer request payloads,
